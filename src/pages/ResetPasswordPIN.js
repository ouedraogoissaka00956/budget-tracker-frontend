...
const ResetPasswordPIN = () => {
  ...
  useEffect(() => {
    // Récupérer l'email et le PIN depuis la navigation
    if (location.state?.email && location.state?.pin) {
      setEmail(location.state.email);
      setPin(location.state.pin);
    } else {
      // Si pas d'email/pin, rediriger vers forgot-password
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await authService.resetPasswordWithPIN(email, pin, formData.password);
